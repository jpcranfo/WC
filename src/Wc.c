...
/*---Highlights of the program
	1)  Technical Highlight:  Dynamic Memory Allocation:  Functions AllOneDDoubleArr, AllOneDIntArr, AllTwoDChaArr, and AllOneDChaArr allocate small initial amount of memory for arrays, then grows the memory reserved for the array dynamically as words and lines of text file are processed.
	2)  Technical Highlight:  Search algorithm:  I use the binary search algorithm to search through sorted arrays for insertion point more efficiently than just starting at beginning of array and going element by element.
---*/
...
/*---Declaration of functions
	...
	AllTwoDChaArr				Allocates or reallocates memory of a 2D array of characters representing array of words (WordArr), returns pointer to allocated WordArr
...
int main()
{
	...
	int *NumChaArr;							//1D Array of integers, representing the number of characters/word for each word
	int *WordCountArr;						//1D Array of integers, representing the number of times word appears, for each word
	int *NumWPLArr;							//1D Array of integers, representing the number of words/line (WPL) for each line
	int *NumWPLArrSort;						//1D Array of integers, representing the number of words/line (WPL) for each line, sorted
	double *MedianArr;						//1D Array of doubles, representing the running median as process lines
...
printf( "\n\n|--------------------------------------------------PROCESSING WORDS IN TEXT FILE--------------------------------------------------|\n\n" );
	do																//Loop to count words and find running median until reach EOF
	{
		...
		if ( FinishedLine )													//If finished processing all words in line, then calculate running median
		{
			...
			FinishedLine = 0;												//Reset whether or not finished with reading words from current line to 0, which means not yet finished
		}
...
char **AllTwoDChaArr( char **WordArr, int WordLength, int *MaxNumWor, int *MaxNumCha, int *NumChaArr, int NumWor, int ResWor, int ResCha )
{
	...
	{
		tmp = (char **)malloc( *MaxNumWor*sizeof(char *) );
		...
		/*---Allocate memory for rows in WordArr, and initialize to the same row number in WordArr, but now have more storage space/row for longer words---*/
		for ( row=0; row<*MaxNumWor; row++ )
		{
...
int *CreateSpaceCurrWPL( int *NumWPLArrSort, int MaxNumLines, int NumLines, int InsIndWPL )
{
		...
		if ( NumLines > 0 )
		{
			for ( ind=NumLines; ind>=InsIndWPL; ind-- )
			{
				...
				{
					fprintf( stderr,"\n\n\nERROR - unable to allocate required memory for NumWPLArrSort (1D array to hold ordered list of number words per line), try freeing up some memory by closing other programs and try again.  Now exiting\n\n\n" );
					exit( 1 ) ;
				}
			}
		}
...
char **CreateSpaceForWord( char **WordArr, int *MaxNumWor, int NumWor, int InsIndWord, int *NumChaArr, int *WordCountArr )
{
		...
		if ( NumWor > 0 )
		{
			for ( ind=NumWor; ind>=InsIndWord; ind-- )
			{
				...
				{
					fprintf( stderr,"\n\n\nERROR - unable to allocate required memory for WordArr (2D array to hold words), try freeing up some memory by closing other programs and try again.  Now exiting\n\n\n" );
					exit( 1 ) ;
				}
				
				if ( NumChaArr == NULL )							//Checking for memory allocation errors
				{
					fprintf( stderr,"\n\n\nERROR - unable to allocate required memory for NumChaArr (1D array to hold number of characters for each word), try freeing up some memory by closing other programs and try again.  Now exiting\n\n\n" );
					exit( 1 ) ;
				}
				
				if ( WordCountArr == NULL )							//Checking for memory allocation errors
				{
					fprintf( stderr,"\n\n\nERROR - unable to allocate required memory for WordCountArr (1D array to hold number of times each word appears in text files), try freeing up some memory by closing other programs and try again.  Now exiting\n\n\n" );
					exit( 1 ) ;
				}
			}
		}
